    ...
    try {
        ...
        if (rentalsList) {
            rentalsList.addEventListener('click', (e) => {
                ...
                const card = target.closest('.rental-card');
                if (!card)
                    return;
...
function generateRentalSummaryPDF(rental) {
    ...
    const formatDate = (dateStr) => dateStr ? new Date(dateStr).toLocaleDateString('tr-TR') : 'Belirtilmemiş';
    ...
    if (savedData) {
        try {
            ...
            if (appData.vehiclesData)
                vehiclesData = appData.vehiclesData;
            if (appData.customersData)
                customersData = appData.customersData;
            if (appData.rentalsData)
                rentalsData = appData.rentalsData;
            if (appData.reservationsData)
                reservationsData = appData.reservationsData;
            if (appData.maintenanceData)
                maintenanceData = appData.maintenanceData;
